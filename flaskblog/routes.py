...
from flaskblog.forms import RegistrationForm, LoginForm, UpdateAccountForm, PostForm, RestPasswordForm, RestRequestForm
...
def register():
    ...
    if current_user.is_authenticated:
        ...
        if form.validate_on_submit():
            ...
            db.session.add(user)
            db.session.commit()
            flash(f'Account created succesfully for {form.username.data}.You can now log in', 'success')
...
def send_reset_email(user):
    ...
    if user.email == 'allansifuna324@gmail.com':
        ...
        if you did not make this request please ignore.Thank you!
        """
    else:
        msg = Message('Password Reset Request', sender='noreply.sifuna@gmail.com', recipients=[user.email])
        msg.body = f"""
